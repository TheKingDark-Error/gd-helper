...
public:
    ...
    void stopRecord() { m_recording = false; }
    ...
    void update(float dt) {
        ...
        if (m_playing && m_layer) {
            ...
            if (m_playIndex >= m_actions.size()) m_playing = false;
        }
    }
    void clear() {
        m_actions.clear();
...
public:
    ...
    void onFastStep(CCObject*) {
        ...
        for (int i = 0; i < 5; i++) m_layer->stepUpdate();
    }
...
public:
    ...
    void applySpeed() {
        ...
        if (scheduler) scheduler->setTimeScale(m_active ? m_speed : 1.0f);
    }
    void reset() {
        m_active = false;
        applySpeed();
